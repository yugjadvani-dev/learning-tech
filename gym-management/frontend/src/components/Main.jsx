import LoginAndRegisterButton4 from "./LoginAndRegisterButton4";
const Main = ({ className = "" }) => {
  return (
    <form
      className={`m-0 w-[406px] flex flex-col items-start justify-start gap-[41.7px] max-w-[calc(100%_-_835px)] shrink-0 mq1025:hidden mq1125:max-w-full mq450:gap-[21px] ${className}`}
    >
      <h1 className="m-0 w-[146px] relative text-17xl font-bold font-poppins text-midnightblue-100 text-left inline-block mq1025:text-10xl mq450:text-3xl">
        Sign-in
      </h1>
      <div className="self-stretch h-[231px] flex flex-col items-start justify-start gap-[3.4px] max-w-full">
        <div className="w-[146px] flex flex-row items-start justify-start pt-0 px-0 pb-[2.6px] box-border">
          <h3 className="m-0 flex-1 relative text-5xl font-bold font-poppins text-darkslateblue-500 text-left mq450:text-lgi">
            Email*
          </h3>
        </div>
        <div className="self-stretch flex-1 flex flex-row items-start justify-start pt-0 px-0 pb-[12.4px] box-border max-w-full">
          <input type="email" className="self-stretch flex-1 relative rounded-3xs bg-white border-darkslateblue-500 border-[4px] border-solid box-border max-w-full" />
        </div>
        <h3 className="m-0 w-[146px] h-[30.8px] relative text-5xl font-bold font-poppins text-darkslateblue-500 text-left inline-block shrink-0 mq450:text-lgi">
          Password*
        </h3>
        <input
